...
from main import (InvalidBookType, check_for_redirect, download_image,
                  save_json, download_txt, get_book, parse_book_page)
        ...
        try:
            ...
            for book_num in book_numbers:
                book_url = urllib.parse.urljoin(
                    f'https://tululu.org/l55/{book_num}', book_num.select_one('a')['href'])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--start_page", default=0, type=int,
                        help="Book download start page.")
    parser.add_argument("--end_page", default=5, type=int,
                        help="Book download end page.")
    parser.add_argument("--dest_folder", default='fantasy_books', type=str,
                        help="Directory path.")
    ...
    for book_url in links:
        try:
            ...
            book_page_num = urllib.parse.urlparse(
                book_url).path.split('/')[1].split('b')[1]

            if not args.skip_imgs:
                download_image(book_page_num, book_image, dest_folder)
            if not args.skip_txt:
                download_txt(get_book(
                    f'https://tululu.org/txt.php', book_page_num), book_name, dest_folder)
        except (requests.exceptions.HTTPError, requests.exceptions.ConnectionError) as ex:
            print(ex)
            time.sleep(60)
