...
def parse_book_page(response):
    ...
    for count, tags in enumerate(comments):
        ...
        for tag in tags:
            book_comments.update({f"comment_{count}": tag.text})
    ...
    for genre in genres_search:
        ...
        for key, item in enumerate(genres):
            ...
            book_genres.update({f"genre_{key+1}": item[0]})
...
def save_json(book_components, path="json"):
    pathlib.Path(path).mkdir(parents=True, exist_ok=True)
    with open(
        os.path.join(path, "books_content.json"), "w+", encoding="utf-8-sig"
    ) as f:
...
def download_image(book_id, book_image, path="images"):
    ...
    with open(os.path.join("media", path, img_name), "wb") as file:
    ...
    for book_id in range(start_id, end_id):
        try:
            ...
            if not args.skip_imgs:
                download_image(book_id, book_image)

            if not args.skip_txt:
                download_txt(
                    get_book(f"https://tululu.org/txt.php", book_id),
                    book_name,
                    dest_folder,
                )
        except (
